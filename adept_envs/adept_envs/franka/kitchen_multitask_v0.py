...
class KitchenV0(robot_env.RobotEnv):
   ...
   def __init__(self, robot_params={}, frame_skip=40):
       ...
       self.tasks_to_complete = ["light switch", "slide cabinet", "microwave", "kettle"]
       self.step_task_completions = set()
       self.obs_dict = {}
   ...
   def compute_reward(
       ...
       for task in self.tasks_to_complete:
           distance = np.linalg.norm(achieved_goal[task] - OBS_ELEMENT_GOALS[task])
           #print(f'{task} Distance: {distance}')
           complete = distance < BONUS_THRESH
           if complete:
               self.step_task_completions.add(task)
   ...
   def step(self, a, b=None):
       ...
       achieved_goal = {
           task: self.obs_dict['qpos'][OBS_ELEMENT_INDICES[task]] for task in self.tasks_to_complete
       }


       reward = self.compute_reward(achieved_goal)
       ...
       # print(f'Cureent States: {self.obs_dict["qpos"].astype(np.float32)} {len(self.obs_dict["qpos"])}')
       # print(f'Goal States: {self.goal} {len(self.goal)}')
       # self.render()
       ...
       return obs, reward, reward == 4, env_info
   ...
   def _get_obs(self):
       ...
       self.obs_dict = {}
       self.obs_dict['qpos'] = self.sim.data.qpos
       self.obs_dict['t'] = t
       self.obs_dict['qp'] = qp
       self.obs_dict['qv'] = qv
       ...
       if self.goal_concat:
           return np.concatenate([self.obs_dict['qp'], self.obs_dict['obj_qp'], self.obs_dict['qpos'], self.obs_dict['goal']])
...
class KitchenTaskRelaxV1(KitchenV0):
   ...
   #     for task_id, task in enumerate(achieved_goal):
   #         distance = np.linalg.norm(achieved_goal[task_id] - desired_goal[task_id])
   #         complete = distance < 0.3
   #         if complete:
   #             self.step_task_completions.append(task_id)


   #     return float(len(self.step_task_completions)), float(len(self.step_task_completions))
